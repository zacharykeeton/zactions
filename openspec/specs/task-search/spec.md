...
Clicking a search result SHALL navigate to the task's list, expand collapsed parent tasks, scroll to the task, highlight it briefly, and clear the search query.

#### Scenario: Click result navigates to list
- **WHEN** the user clicks a search result for a task in the "Work" list
- **THEN** the active list filter switches to "Work", collapsed ancestors expand, the task scrolls into view with a highlight animation, and the search is cleared
